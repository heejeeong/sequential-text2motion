...
from models.encdec_pe import TemporalAutoregressiveEncoder, Decoder
...
class VQVAE_bodypart(nn.Module):
    def __init__(self,
        ...
        if args.dataname == 't2m':
            ...
            for name in self.parts_name:
                ...
                encoder = TemporalAutoregressiveEncoder(raw_dim, output_dim, down_t, stride_t, hidden_dim, depth, dilation_growth_rate, activation=activation, norm=norm)
    ...
    def forward(self, parts, time_indices=None):
        """
        Forwarding.
        ...
        for i, name in enumerate(self.parts_name):  # parts_name: ['Root', 'R_Leg', 'L_Leg', 'Backbone', 'R_Arm', 'L_Arm']
            ...
            # Encode
            encoder = getattr(self, f'enc_{name}')
            x_encoder = encoder(x_in, time_indices=time_indices) 
...
class HumanVQVAEBodyPart(nn.Module):
    ...
    def forward(self, x, time_indices=None):
        x_out_list, loss_list, perplexity_list = self.vqvae(x, time_indices=time_indices)
        return x_out_list, loss_list, perplexity_list
