...
from dataset import dataset_VQ_bodypart_pe, dataset_TM_eval_bodypart
from models import vqvae_bodypart_pe as vqvae
...
train_loader = dataset_VQ_bodypart_pe.DATALoader(args.dataname,
                                        args.batch_size,
                                        window_size=args.window_size,
                                        unit_length=2**args.down_t)

train_loader_iter = dataset_VQ_bodypart_pe.cycle(train_loader)
...
for nb_iter in range(1, args.warm_up_iter):
    ...
    pos_encoding = gt_parts[-1].cuda().long()  # (window_size, 1)
    ...
    #print("len(gt_parts):", len(gt_parts))

    pred_parts, loss_commit_list, perplexity_list = net(gt_parts, time_indices=pos_encoding)


    #pred_parts, loss_commit_list, perplexity_list = net(gt_parts)

    pred_parts_vel = dataset_VQ_bodypart_pe.get_each_part_vel(
        pred_parts, mode=args.dataname)
    gt_parts_vel = dataset_VQ_bodypart_pe.get_each_part_vel(
...
for nb_iter in range(1, args.total_iter + 1):
    ...
    pos_encoding = gt_parts[-1].cuda().long()  # seperate positional encoding
    gt_parts = gt_parts[:-1]  # left 6 motions
