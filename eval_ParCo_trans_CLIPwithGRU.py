...
# === Add this: GRU-based encoder ===
if args.use_clip_gru_concat:
    class CLIP_GRU_Encoder(torch.nn.Module):
        def __init__(self, clip_model, clip_dim=512, hidden_dim=512, num_layers=1):
            super().__init__()
            self.clip_model = clip_model
            self.clip_dim = clip_dim
            self.hidden_dim = hidden_dim
            self.gru = torch.nn.GRU(input_size=clip_dim, hidden_size=hidden_dim, num_layers=num_layers, batch_first=True)
            self.final_proj = torch.nn.Linear(clip_dim + hidden_dim, clip_dim)

        def forward(self, text):
            with torch.no_grad():
                x_clip = self.clip_model.token_embedding(text)
                x_clip = x_clip + self.clip_model.positional_embedding[:x_clip.size(1), :]
                x_clip = self.clip_model.ln_final(x_clip)
