...
import options.option_transformer_bodypart_CLIPwithGRU as option_trans
from options.option_transformer_bodypart_CLIPwithGRU import trans_bodypart_cfg
...
import models.t2m_trans_bodypart_CLIPwithGRU as trans_bodypart
...
# train_ParCo_trans_0522.py 상단 또는 적절한 위치에 추가
class CLIP_GRU_Encoder(nn.Module):
    def __init__(self, clip_model, clip_dim=512, hidden_dim=512, num_layers=1):
        super().__init__()
        self.clip_model = clip_model
        self.clip_dim = clip_dim
        self.hidden_dim = hidden_dim
        self.gru = nn.GRU(input_size=clip_dim, hidden_size=hidden_dim, num_layers=num_layers, batch_first=True)
        self.final_proj = nn.Linear(clip_dim + hidden_dim, clip_dim)

    def forward(self, text):
        with torch.no_grad():
            x_clip = self.clip_model.token_embedding(text) + self.clip_model.positional_embedding  # (B, 77, 512)
            x_clip = x_clip + self.clip_model.positional_embedding[:x_clip.size(1), :]  # (B, 77, 512)
            x_clip = self.clip_model.ln_final(x_clip)  # (B, 77, 512)
        gru_out, _ = self.gru(x_clip)
        concat_feat = torch.cat([x_clip, gru_out], dim=-1)
        fused_feat = self.final_proj(concat_feat)
        #print("[DEBUG] final fused_feat shape:", fused_feat.shape)
...
# ---- Replace CLIP encode_text with GRU-augmented version ----
if args.use_clip_gru_concat:
    text_encoder = CLIP_GRU_Encoder(clip_model).cuda()
    clip_model.encode_text = text_encoder  
...
while nb_iter <= args.total_iter:
    ...
    clip_tokens = clip_model.encode_text(text)  # (B, 77, 512)
    #print("[DEBUG] clip_tokens shape:", clip_tokens.shape)
    feat_clip_text = clip_tokens.float()       # (B, 512) 
    #print("[DEBUG] feat_clip_text shape:", feat_clip_text.shape)
