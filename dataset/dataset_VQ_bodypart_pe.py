...
class VQMotionDatasetBodyPart(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        # 추가: positional encoding 생성
        pos_encoding = np.arange(self.window_size)  # [0, 1, 2, ..., window_size-1]
        pos_encoding = torch.from_numpy(pos_encoding).float().unsqueeze(1)  # (window_size, 1)


        Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm = parts  # explicit written code for readability
        return [Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm, pos_encoding]
